...
import { CreateEvent, UpdateEvent, AllCurrentEvent } from './api_views/events_apis.js'
...
app.get('/current_event/all', AllCurrentEvent);
...
io.on("connection", (socket) => {
    ...
    socket.on("CUSTOMER_REQUEST", (data) => {
        ...
        try {
            console.log(data.provider.provider_id);  // Access property using correct notation
            io.emit(`SERVICE_REQUEST:${data.provider.provider_id}`, data);
        } catch (err) {
            console.log(err);
        }
    });
